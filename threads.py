...
class BigWorkThread(QtCore.QThread):
    ...
    #重写 run() 函数，在里面干大事。
    def run(self):
        self.read_excel( 2,  self.inpath, self.exportFilePath)

    def stop(self):
        print("终止线程")
        self.flag = False

    def read_excel(self,begin_row,  filePath, exportFilePath):
        self.finishSignal.emit(['正在加载EXCEL，根据文件大小，载入需要几分钟时间，请稍候...'])
        print("loading excel")
        rule = {
            1: "长度必须为18位",
        ...
        try:
            ...
            col=validateUtil.findCol(ws,col_name)
            # 显示表名，表行数，表列数
            ...
            for i in range(begin_row, rows + 1):
                if self.flag:
                    ...
                    vcell=ws.cell(row=i, column=col)
                    value = vcell.value
